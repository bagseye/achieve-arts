...
function loadGoogleMaps(apiKey) {
	...
	window.__bwpGoogleMapsPromise = new Promise((resolve, reject) => {
		...
		if (document.getElementById(scriptId)) {
			...
			setTimeout(() => {
				clearInterval(timer);
				reject(new Error('Google Maps script present but did not initialise'));
			}, 8000);
			...
			$maps.forEach((el) => {
				...
				new window.google.maps.Marker({
					...
					icon: markerUrl
						? {
								url: markerUrl,
								scaledSize: new window.google.maps.Size(markerSize[0], markerSize[1]),
								// Optional: if your SVG is “pointy” at the bottom, anchor it:
								anchor: new window.google.maps.Point(markerSize[0] / 2, markerSize[1]),
							}
						: undefined,
				});
			});
